...
fun main() = runBlocking {
    ...
    AliasClient(FixtureAdapter(connection)).use { client ->
        check(client.connection() == connection)
        val created = client.create(CreateAliasRequest("example.test"))
        check(created.identity == identity)
        check(client.list(ListAliasesRequest(null)).aliases == listOf(created))
        check(client.get(identity) == created)
        check(client.setEnabled(identity, false).lifecycle == AliasLifecycleState.DISABLED)
        val reply = client.createSendReplyIdentity(
            CreateSendReplyIdentityRequest(identity, "recipient@example.test"),
        )
        check(reply.identityId == "reply/object:9")
        check(client.listSendReplyIdentities(identity, null).identities == listOf(reply))
        val blocked = client.setSendReplyBlocked(reply, true)
        check(blocked == reply.copy(blocked = true))
        client.removeSendReplyIdentity(blocked)
        check(client.delete(identity).deleted)
    }
    ...
    check(state.resources.single().identity.aliasId == identity.aliasId)
    check(state.resources.single().tombstoned)
...
private class FixtureAdapter(
    ...
    private fun alias(enabled: Boolean) = Alias(
        identity = AliasIdentity(1u, connection.connectionId, "remote/object:7", "alias@example.test"),
